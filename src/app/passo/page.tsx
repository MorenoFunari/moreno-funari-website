...
export default function PassoPage() { return <main className={styles.page} id="main-content">
  ...
  <section className={styles.section}><Container><div className={styles.copy}><h2>Cosa trovi dentro</h2></div><ul className={styles.list}><li>5 passaggi semplici</li><li>Domande pratiche</li><li>Esempi di vita reale</li><li>Uno spazio per riconoscere i sì automatici</li><li>Un primo passo per mettere un confine sostenibile</li></ul><SurfaceCard className={styles.card}><h3>Ricevi la guida gratuita</h3><p>Ti chiedo solo i dati necessari per inviarti la guida. Potrai leggere tutto con calma e decidere tu se restare in contatto.</p><TrackedCtaLink eventName="cta_click_passo" external href={brevoFormUrl} location="passo_form_fallback" size="large">Ricevi la guida via email</TrackedCtaLink><p>Il modulo è gestito da Brevo. Leggi la <Link href="/privacy-policy">Privacy Policy</Link>.</p></SurfaceCard></Container></section>
